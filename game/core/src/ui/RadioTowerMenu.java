...
{
	private static final int DELTA_THRESHOLD = 64;
	...
	public RadioTowerMenu(Terrain Ter)
	{
		ter = Ter;
		pos = new Vector2();
		isvalid = false;
	...
	public void setSelectedTarget(Squad Selected)
	{
		...
		int xpos = 0;
		if (d == 1) {
			xpos = (int)pos.x + RadioTower.Tower.getWidth()/2 + (int)Selected.getBoundingBox().width/2;
		} else {
			xpos = (int)pos.x + RadioTower.Tower.getWidth()/2 - (int)Selected.getBoundingBox().width/2;
		}
	...
	{
		// TODO - decide if you would like it to use this
	...
	{
		float xpos = Cursor.getMouseX(Cam.getPos()) + Cam.getPos().x - RadioTower.Tower.getWidth()/2;
		...
		if (pos.x != xpos) {
			...
			int miny = ter.getMinHeight( (int)pos.x, (int)(pos.x + RadioTower.Tower.getWidth()) );
			int maxy = ter.getMaxHeight( (int)pos.x, (int)(pos.x + RadioTower.Tower.getWidth()) );
	...
	{
		float width = RadioTower.Tower.getWidth()/2f;
	...
	public void render(SpriteBatch Batch, Camera Cam)
	{
		...
		Batch.draw(RadioTower.Tower, Cam.getRenderX(pos.x), Cam.getRenderY(pos.y));
